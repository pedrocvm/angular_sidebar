(function(angular){
    ...
    .controller('ngImgMapCtrl', ['$scope', 'ngImgMapCalculation', 'ngImgMapCurArea', 
        ...
        function init(){
            ...
            if (m.pic_url != null && m.pic_url != "") {
                var i = new Image();
                i.src = m.pic_url;

                i.onload = function () {
                    $scope.loadImage([i.width, i.height], canSize, m);
                    $scope.$apply();
                }
            }

            if (calculation == null)
                $scope.loadImage([0, 0], canSize, m);
        }

        $scope.loadImage = function (imgSize, canSize, m) {
            // 初始化新实例
            calculation = new ngImgMapCalculation(imgSize, canSize);
            ratio = calculation.ratio;
            ...
            // 修正坐标
            let me = this;
            if (angular.isDefined(m) && me.imgw > 0) {
                angular.forEach(m.maps, function (area) {
                    calculation.checkCoords(area.coords);
                })
            };
